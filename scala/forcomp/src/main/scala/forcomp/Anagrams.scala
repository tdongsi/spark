...
object Anagrams {
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    dictionary.groupBy(str => wordOccurrences(str)) withDefaultValue(List())
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val occurs = occurrences.map( x => (for (i <- 1 to x._2) yield (x._1, i)).toList )
    occurs.foldRight(List[Occurrences](Nil))( (x, y) => y ++ (for (i <-x; j <-y) yield i :: j))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def helper(occs: Occurrences): List[Sentence] = {
      if (occs.isEmpty) List(List())
        ...
        for {
          a <- combinations(occs)
          b <- dictionaryByOccurrences(a)
          c <- helper(subtract(occs, a))
        } yield b :: c
    }
